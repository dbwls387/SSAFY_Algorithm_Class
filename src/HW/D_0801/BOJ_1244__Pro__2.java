...
public class BOJ_1244__Pro__2 {

	static int N, COUNT;
	static boolean[] switches; // 스위치 배열
...
//			switches[i] = Integer.parseInt(st.nextToken()) == 1 ? true : false;
			if (switches[i] = st.nextToken().equals("1"))
				switches[i] = true; // false는 boolean의 default
		}
...
//			if (switches[num - cnt] == switches[num + cnt]) { // 같으면
...
//		}
		...
		while (left >= 1 && right <= N) {
			if (switches[left] == switches[right]) { // 같으면
