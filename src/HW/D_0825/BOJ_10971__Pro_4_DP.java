...
// #1 모든 도시를 맨 앞에 두면서 따질 필요가 없다. 순환 구조 3-1-2-5-4-(3) 이므로 맨 앞의 도시를 하나만 따져도 된다. 
// #2 DP 점진적으로 계산한 결과를 저장 
// 	  dp[i][j] : 현재 i번 도시에 있고, 거쳐온 도시들이 j이다. 값은 남은 도시를 방문하는데 필요한 최소 비용 
...
// 계산 방법 
//	  dp[3][1100101] => 1,3,6,7  방문 후, 남은 2,4,5 도시를 방문하는 최소 비용 (이후에 2번을 가는 경우, 또는 4번을 가는 경우, 또는 5번을 가는 경우를 모두 따져서 최소값 갱신) 
// 	  // 2번 도시를 간다고 가정 (4번도 가봐야 하고, 5번도 가봐야 한다. )
// 	  2번 : dp[2][1100111]   << 만약 4번을 먼저 간다면 dp[4][1101101] / 5번을 먼저 간다면 dp[5][1110101] >> 
//    dp[3][1100101] ==> [ dp[2][1100111] + 3 -> 2 비용 ] [ dp[4][1101101] + 3 -> 4 비용 ] [ dp[5][1110101] + 3 -> 비용 ] 중 최소 비용 선택 
...
public class BOJ_10971__Pro_4_DP {
