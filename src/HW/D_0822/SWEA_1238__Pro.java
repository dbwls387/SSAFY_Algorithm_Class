...
public class SWEA_1238__Pro {

	static int L, S, maxNum, maxDepth;
	static boolean[][] matrix = new boolean[101][101]; // 0 dummy (visit으로 활용)
	...
	static void bfs() {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 1; i <= 100; i++) {
				// vertex.n 정점에서 갈 수 있는지 i ~ 100번까지 모든 정점을 고려
				// i 정점 visit 체크
				if (!matrix[vertex.n][i] || matrix[i][0])
					continue;
				matrix[i][0] = true;
