...
public class BOJ_17135_Pro {

	static int N, M, D, max;
	static int[][] map;
	static int[] archer = new int[3]; // x 좌표만 필요 // 조합에서 tgt[] 역할을 함
	...
	public static void main(String[] args) throws Exception {
		...
		// 조합이 완성되면 시뮬레이션 진행 -> 새로운 조합이 완성되면 시뮬레이션 다시 진행 (반복)
		comb(0, 0);
	...
	static void check() {
		...
		while (true) {
			...
			for (int i = 0; i < 3; i++) {
				...
				// PriorityQueue를 이용해서 가장 가까운 적 선별
				pqueue.clear();
				int ac = archer[i]; // 첫번째 궁사 ~~ 세번째 궁사 (위치 X 좌표)
				...
				for (int j = 0; j < size; j++) {
					...
					if (e.d <= D) { // 사정거리 안에 드는 적만 pqueue에 담는다.
						pqueue.offer(e);
					}
				}

				// pqueue가 비워있지 않으면 제거 대상이 있다. -> 표시
				if (!pqueue.isEmpty()) {
	...
	static class Enemy {
		...
		public Enemy(int y, int x) { // d, dead는 문제 풀이 과정에서 set
			this.y = y;
			this.x = x;
		}
	}
