...
// 프림
// 정점 중심 
// 최소비용의 정점을 선택하면서 완성, 선택된 정점으로부터 갈 수 있는 정점들을 모두 고려 대상으로 ! (visit) 

public class BOJ_17472__Pro_2_Prim {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < M; j++) {
				map[i][j] = Integer.parseInt(st.nextToken()) * (-1); // 육지를 -1로 <= 정점번호를 1부터 하기 위해서 ...
			}
		}
		...
		while (!pqueue.isEmpty()) {
			...
			if (visitPrim[edge.v]) // 이미 선택된 정점은 skip
				continue;
