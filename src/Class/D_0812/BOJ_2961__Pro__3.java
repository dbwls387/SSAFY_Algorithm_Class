...
public class BOJ_2961__Pro__3 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i < count; i++) { // 모든 부분집합에 대해서	
			// ㄴ 재료는 한가지 이상 선택이기 때문에 i=1 부터 출발
			
			// 각각의 부분집합에서 어떤 원소가 선택 / 비선택 되었는지 확인
			// i 가 select 역할을 한다. ( i는 단순한 count 변수가 아니다. )
			...
			for (int j = 0; j < N; j++) {
				if ((i & (1 << j)) != 0) { // i와 비교해보니 j가 선택되었다 확인
					sin *= src[j][0];
					ssn += src[j][1];
				}
			}
