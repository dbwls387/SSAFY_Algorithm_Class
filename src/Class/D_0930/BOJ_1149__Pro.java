...
public class BOJ_1149__Pro {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 2; i <= N; i++) {
			...
			// i번째에 G를 선택할 경우, i-1 번째에 R, B가 선택되어야 한다. 이 중 최소 비용을 선택
			memoi[i][1] = Math.min(memoi[i - 1][0], memoi[i - 1][2]) + cost[i][1];

			// i번째에 B를 선택할 경우, i-1 번째에 R, G가 선택되어야 한다. 이 중 최소 비용을 선택
			memoi[i][2] = Math.min(memoi[i - 1][0], memoi[i - 1][1]) + cost[i][2];
		}

		// 3가지로 배열을 만들어서 따져본 것은 처음에 어느색을 선택했을 때 가장 최소 비용인지 모르기 때문에 세가지 색을 모두 따져본다.
