...
// bfs로 물이 퍼져 나갈 때, while (큐가 비우지 않는 동안) { ... 물이 퍼지는 반복 <- 고슴도치의 이동이 포함 }
// 고슴도치, 물 따로 Queue를 생성하고, 고슴도치의 Queue를 중심으로 처리 
// 		=> Queue에 대한 작업을 현재 담긴 모든 대상을 전부 다 처리를 다 하고 고슴도치 처리 다 하고... 

public class BOJ_3055__Pro {
	...
	static void check() {
		...
		while (!gQueue.isEmpty()) { // 1분 행위
			...
			// 고슴도치 이동
			// 물 확산을 이미 처리했으므로 고슴도치는 이동시에 빈 칸인 것만으로 이동 ('D' 이면 종료)
			size = gQueue.size();
