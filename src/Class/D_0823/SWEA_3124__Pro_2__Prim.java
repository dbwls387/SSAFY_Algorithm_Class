...
public class SWEA_3124__Pro_2__Prim {
	...
	static void prim() {
		...
		pqueue.addAll(vertex.get(1)); // 시작 정점 (1번) 에서 갈 수 있는 모든 정점 (간선)을 pq에 담는다.

		while (!pqueue.isEmpty()) {
			Edge edge = pqueue.poll();
			if (visit[edge.v]) // 새로 최소비용으로 연결하려는 정점이 이미 방문 (연결)한 정점이면 skip
				continue;
	...
	static class Edge {	// Vertex로 이해하는 게 더 편함 
		int v, c;
